...
class mpc_controller:
    ...
    def __init__(self, T, N, r_dim, v_max, omega_max, Q, R, n_obs):
        self.T = T
        self.N = N
        self.r_dim = r_dim
        self.v_max = v_max
    ...
    def setup_controller(self):
        ...
        self.opt_obs_x = self.opti.parameter(1,self.n_obs)
        self.opt_obs_y = self.opti.parameter(1,self.n_obs)
        for i in range(self.N+1):
            for j in range(self.n_obs):
                temp_constraints_ = ca.sqrt((self.opt_states[i, 0]-self.opt_obs_x[j] - 0.15)**2+(self.opt_states[i, 1]-self.opt_obs_y[j] - 0.15)**2)
                self.opti.subject_to(self.opti.bounded(self.obs_diam/2 + self.r_dim/2 - 0.15, temp_constraints_, np.inf))
    ...
    def solver(self, next_trajectories, next_controls,obs_x, obs_y):
        # Set parameter, here only update initial state of x (x0)
        self.opti.set_value(self.opt_x_ref, next_trajectories)
        self.opti.set_value(self.opt_u_ref, next_controls)
        self.opti.set_value(self.opt_obs_x, obs_x)
        self.opti.set_value(self.opt_obs_y, obs_y)
        ...
        # Solve the problem
        sol = self.opti.solve()
        print(self.opti.debug.value(self.opt_obs_x))
        print(self.opti.debug.value(self.opt_obs_y))
